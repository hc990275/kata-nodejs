...
| :--- | :--- | :--- | :--- | :---: |
| **Lunes Host** | `128 MB` / `512 MB` / `15%` | 🔄 每 15 天须登录控制台续期 |  | 🟢 正常 |
| **Katabump** | `308 MB` / `716 MB` / `25%` | 🔄 每 4 天须手动续期 |  | 🟢 正常 |
| **Bot Hosting** | `256 MB` / `1 GB` / `20%` |🔄 每日赚取 10 金币，自动扣费 | ❌ 不开放 7844 端口<br>❌ 不支持 Cloudflare Argo | 🟢 正常 |
| **Pella.app** | `100 MB` / `5 GB` / `10%` | 🔄 免费机型 24 小时须续期 | ❌ 无公网 TCP/UDP 端口<br>✅ **仅支持 Argo 临时/固定隧道** | 🟢 正常 |
| **Freecloudpanel** | `64 MB` / `64 MB` / `100%` | 🔄 每 30 天须手动续期 | 🚫 不定期开放注册 | 🟡 稀缺 |
| **Zenix.sg** | `2 GB` / `10 GB` / `100%` | ⏳ 创建需要长时间排队 | ⏳ 目前正在队列等待中... | ⏳ 漫长排队 |
